...
def translate(functions: List[Function], with_debug: bool = False) -> TranslationResult:
    ...
    def run_translation(func_name, instr_shift):
        subtranslator = SingleFunctionTinyTranslator(all_functions)
...
class SingleFunctionTinyTranslator:
    ...
    def calc_and_alloc(self, name, type, array_size):
    ...
    def _parse_atom(self, atom: Atom):
        if isinstance(atom.value, IntegerLiteral):
            ...
            if isinstance(atom.value.index, int):
                self.save_instr(LoadI(array_start - atom.value.index))
                self.stack_pos += 1
            elif isinstance(atom.value.index, str):
                # Points to array start + 1
                self.save_instr(PushI(array_start + 1))
            ...
            self.calc_and_alloc(f"{called_func.name}_res", called_func.return_type, called_func.return_array_size)
            ...
            if isinstance(line, Assignment):
                ...
                if isinstance(line.target, str):
                    ...
                    if isinstance(line.target.index, int):
                        self.save_instr(StoreI(array_start - line.target.index))
                        self.stack_pos -= 1
                ...
                var = line.variable
                space = self.calc_and_alloc(var.name, var.type, var.array_size)
